...
require_once __DIR__ . '/../models/scout/Post.php';
...
/* ============== Delete Request (AJAX) ============== */
function scoutDeleteRequestCtrl($pdo) {
    requireScoutAccess();
    ...
    if (!postRequestBelongsToUser($pdo, $requestId, $_SESSION['user_id'])) {
        echo json_encode(['success' => false, 'message' => 'Request not found or access denied']);
        exit;
    }

    $request = findPostRequestById($pdo, $requestId);
    if (!$request) {
    ...
    if (!empty($request['post_data_decoded']['image']) && file_exists(__DIR__ . '/../' . $request['post_data_decoded']['image'])) {
        unlink(__DIR__ . '/../' . $request['post_data_decoded']['image']);
    }

    if (deletePostRequest($pdo, $requestId)) {
...
/* ============== Request Change for Approved Post (AJAX) ============== */
function scoutRequestChangeCtrl($pdo) {
    requireScoutAccess();
    header('Content-Type: application/json');
    ...
    $data = json_decode(file_get_contents('php://input'), true);
    $postId = intval($data['post_id'] ?? $_POST['post_id'] ?? 0);
    ...
    if (!$post || $post['scout_id'] != $_SESSION['user_id'] || $post['status'] !== 'approved') {
        echo json_encode(['success' => false, 'message' => 'Approved post not found or access denied']);
        exit;
    }
