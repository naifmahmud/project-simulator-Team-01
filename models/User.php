...
function getUsers($conn, $limit = 20, $offset = 0) {
    $stmt = mysqli_prepare($conn, "SELECT id, name, email, role, is_verified, created_at FROM users ORDER BY id DESC LIMIT ? OFFSET ?");
...
function getUser($conn, $id) {
    $stmt = mysqli_prepare($conn, "SELECT id, name, email, role, is_verified, created_at FROM users WHERE id = ?");
    mysqli_stmt_bind_param($stmt, 'i', $id);
    mysqli_stmt_execute($stmt);
    $row = mysqli_fetch_assoc(mysqli_stmt_get_result($stmt));
    mysqli_stmt_close($stmt);
...
function addUser($conn, $name, $email, $password, $role, $is_verified = 0) {
    $passwordHash = password_hash($password, PASSWORD_BCRYPT);
    $stmt = mysqli_prepare($conn, "
        INSERT INTO users (name, email, password_hash, role, is_verified, created_at)
        VALUES (?, ?, ?, ?, ?, NOW())
    ");
    mysqli_stmt_bind_param($stmt, 'ssssi', $name, $email, $passwordHash, $role, $is_verified);
    $ok = mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    return $ok;
}
...
function verifyUserStatus($conn, $id, $status = 1) {
